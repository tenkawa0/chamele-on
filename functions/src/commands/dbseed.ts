...
import { Administrator } from '../services/chamele-on/models/administrator';
import { collectionName } from '../services/chamele-on/constants';
import { addCounter } from '../firestore-admin/record-counter';

import serviceAccount from '../chamele-on-firebase-adminsdk.json';
...
const uploadSeed = async (collection: string, seedFile: string) => {
  ...
  switch (collection) {
    case collectionName.administrators: {
      const docs: Required<Administrator>[] =
        records.map((record: Administrator) => ({
          ...record,
